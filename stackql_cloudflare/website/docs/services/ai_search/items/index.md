...
<tr><td><b>Id</b></td><td><CopyableCode code="cloudflare.ai_search.items" /></td></tr>
</tbody></table>
...
next_action,
status
FROM cloudflare.ai_search.items
...
```sql
INSERT INTO cloudflare.ai_search.items (
file,
metadata,
wait_for_completion,
id,
...
```sql
UPDATE cloudflare.ai_search.items
SET 
...
```sql
REPLACE cloudflare.ai_search.items
SET 
key = '{{ key }}',
next_action = '{{ next_action }}',
wait_for_completion = {{ wait_for_completion }}
...
```sql
DELETE FROM cloudflare.ai_search.items
WHERE id = '{{ id }}' --required
AND item_id = '{{ item_id }}' --required
AND account_id = '{{ account_id }}' --required
AND name = '{{ name }}' --required
