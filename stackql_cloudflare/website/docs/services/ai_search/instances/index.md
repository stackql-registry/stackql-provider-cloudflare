...
<tr><td><b>Id</b></td><td><CopyableCode code="cloudflare.ai_search.instances" /></td></tr>
</tbody></table>
...
sync_interval,
type
FROM cloudflare.ai_search.instances
...
```sql
INSERT INTO cloudflare.ai_search.instances (
ai_gateway_id,
ai_search_model,
cache,
cache_threshold,
...
```sql
REPLACE cloudflare.ai_search.instances
SET 
...
```sql
DELETE FROM cloudflare.ai_search.instances
WHERE account_id = '{{ account_id }}' --required
AND id = '{{ id }}' --required
;
```
...
```sql
EXEC cloudflare.ai_search.instances.ai_search_move_instance 
@account_id='{{ account_id }}' --required, 
@name='{{ name }}' --required, 
@id='{{ id }}' --required 
@@json=
