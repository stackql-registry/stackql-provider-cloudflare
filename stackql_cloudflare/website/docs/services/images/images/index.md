...
<tr><td><b>Id</b></td><td><CopyableCode code="cloudflare.images.images" /></td></tr>
</tbody></table>
...
<tr>
    <td><a href="#create_direct_upload_v2"><CopyableCode code="create_direct_upload_v2" /></a></td>
    <td><CopyableCode code="exec" /></td>
    <td><a href="#parameter-account_id"><code>account_id</code></a></td>
    <td></td>
    <td>Direct uploads allow users to upload images without API keys. A common use case are web apps, client-side applications, or mobile devices where users upload content directly to Cloudflare Images. This method creates a draft record for a future image. It returns an upload URL and an image identifier. To verify if the image itself has been uploaded, send an image details request (accounts/:account_identifier/images/v1/:identifier), and check that the `draft: true` property is not present.</td>
</tr>
</tbody>
</table>
...
<Tabs
    defaultValue="create_direct_upload_v2"
    values={[
        { label: 'create_direct_upload_v2', value: 'create_direct_upload_v2' }
    ]}
>
<TabItem value="create_direct_upload_v2">
...
```sql
EXEC cloudflare.images.images.create_direct_upload_v2 
