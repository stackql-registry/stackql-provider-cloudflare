  ...
  try {
    ...
    for (const service of services) {
      ...
      for (const resource of resources) {
        ...
        const methods = await executeQuery(methodsQuery, `  Getting methods for ${resourceFQRN}`);
        if (!methods || methods.length === 0) {
          console.error(`Error: Resource ${resourceFQRN} has no methods`);
          process.exit(1);
        } else {
          console.log(`Found ${methods.length} methods for ${resourceFQRN}`);
        }
        ...
        if (!hasSelect) {
          ...
          results.nonSelectableResources.push(resourceFQRN);
        }

        // Try DESCRIBE EXTENDED if available.
        //
        // `SHOW METHODS` reports `SQLVerb` heuristically from the HTTP verb,
        // so a resource may show a SELECT-shaped method even when the
        // provider yaml explicitly excludes that method from
        // `sqlVerbs.select` (e.g. when the response body isn't JSON-
        // projectable - raw scripts, images, PDFs, application/scim+json).
        // In that case `DESCRIBE EXTENDED` legitimately returns no columns
        // and stackql replies with "SELECT not supported for this resource".
        // We treat that as a non-selectable resource instead of a fatal
        // error.
        if (resourceData.selectable) {
          try {
            const describeExtendedQuery = `DESCRIBE EXTENDED ${resourceFQRN}`;
            const extendedColumns = await executeQuery(describeExtendedQuery, `  Describing extended ${resourceFQRN}`);

            if (extendedColumns !== null && extendedColumns.length > 0) {
              console.log(`Found ${extendedColumns.length} extended columns for ${resourceFQRN}`);
            } else {
              // 0 rows from DESCRIBE EXTENDED == the resource is not actually
              // selectable. Downgrade to a warning + non-selectable counter.
              console.warn(`  Not actually selectable (DESCRIBE returned no columns) for ${resourceFQRN} - treating as exec-only`);
              results.nonSelectableResourceCount++;
              results.nonSelectableResources.push(resourceFQRN);
            }
          } catch (error) {
            const msg = (error.message || '').toLowerCase();
            if (msg.includes('select not supported')) {
              console.warn(`  Not actually selectable (${error.message}) for ${resourceFQRN} - treating as exec-only`);
              results.nonSelectableResourceCount++;
              results.nonSelectableResources.push(resourceFQRN);
            } else {
              console.error(`Error describing extended ${resourceFQRN}:`, error.message);
              process.exit(1);
            }
          }
        }
